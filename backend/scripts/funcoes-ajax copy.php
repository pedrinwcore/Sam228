...
if($acao == "ligar_streaming") {
	...
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","Streaming ligado com sucesso pelo administrador.");
...
if($acao == "desligar_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","Streaming desligado com sucesso pelo administrador.");
...
if($acao == "reiniciar_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao_streaming($conexao,"".$dados_stm["codigo"]."","Streaming reiniciado com sucesso pelo administrador.");
...
if($acao == "remover_streaming") {
	...
	echo "<span class='texto_status_erro'>Não foi possível remover o streaming ".$dados_stm["login"]."<br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "remover_revenda") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Streaming removido com sucesso na remoção da revenda ".$dados_revenda["id"]." - ".$dados_revenda["email"]."");
	...
	$ssh->executar("nohup rm -rf ".$dados_servidor["path_home"]."/streaming/".$dados_stm_subrevenda["login"]."; echo ok");
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm_subrevenda["login"]."] Streaming removido com sucesso na remoção da sub revenda ".$dados_subrevenda["id"]." - ".$dados_subrevenda["email"]."");
...
if($acao == "alterar_senha_revenda") {
	...
	echo "<span class='texto_status_sucesso'>Senha da revenda <strong>".$dados_revenda["nome"]."</strong> alterada com sucesso para ".$nova_senha."</span><br /><br /><a href='javascript:window.location.reload()' class='texto_status_atualizar'>[Atualizar]</a>";
		...
		// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_revenda["nome"]." - ".$dados_revenda["email"]."] Senha da revenda alterada com sucesso.");
	
	} else {
	
	echo "<span class='texto_status_erro'>Não foi possível alterar a senha da revenda <strong>".$dados_revenda["nome"]."</strong><br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "alterar_senha_streaming") {
	...
	// Insere a ação executada no registro de logs.
	logar_acao($conexao,"[".$dados_stm["login"]."] Alterada senha do stmvideo.");
	
	} else {
	
	echo "<span class='texto_status_erro'>Não foi possível alterar a senha do streaming <strong>".$dados_stm["login"]."</strong><br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "listar_streamings_servidor") {
	...
	$sql = mysqli_query($conexao,"SELECT * FROM streamings where codigo_servidor = '".$codigo_servidor."' ORDER by login ASC");
...
if($acao == "listar_streamings_geral") {
	...
	$sql = mysqli_query($conexao,"SELECT * FROM streamings where status = '1' ORDER by login ASC");
	while ($dados_stm = mysqli_fetch_array($sql)) {
...
if($acao == "manutencao_servidor") {
	...
	if($acao == "ativar") {
	mysqli_query($conexao,"Update servidores set status = 'off', mensagem_manutencao = '".$mensagem."' where codigo = '".$dados_servidor["codigo"]."'");
	} else {
	mysqli_query($conexao,"Update servidores set status = 'on', mensagem_manutencao = '' where codigo = '".$dados_servidor["codigo"]."'");
	}
	...
	echo "<span class='texto_status_erro'>Não foi possível ativar/desativar a manutenção no servidor <strong>".$dados_servidor["nome"]."</strong><br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "remover_dica_rapida") {
	...
	$dados_dica_rapida = mysqli_fetch_array(mysqli_query($conexao,"SELECT * FROM dicas_rapidas where codigo = '".$codigo."'"));

	mysqli_query($conexao,"Delete From dicas_rapidas where codigo = '".$dados_dica_rapida["codigo"]."'");
	mysqli_query($conexao,"Delete From dicas_rapidas_acessos where codigo_dica = '".$dados_dica_rapida["codigo"]."'");
	...
	echo "<span class='texto_status_erro'>Não foi possível remover a dica rápida <strong>".$dados_dica_rapida["titulo"]."</strong><br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "alterar_status_aviso") {
	...
	echo "<span class='texto_status_erro'>Não foi possível alterar o status do aviso <strong>".$dados_aviso["titulo"]."</strong>.</strong><br>Log: ".mysqli_error($conexao)."</span>";
...
if($acao == "remover_aviso") {
	...
	echo "<span class='texto_status_erro'>Não foi possível remover o aviso <strong>".$dados_aviso["titulo"]."</strong>.</strong><br>Log: ".mysqli_error($conexao)."</span>";
